...
const features = [
  ['Cockpit quotidien', 'KPIs, priorités ARK, clients à risque et échéances à 30 jours.', Gauge],
  ['Fiche client 360', 'Identité, contrats, tâches, timeline et recommandations actionnables.', Users],
  ['Contrats', 'Type, compagnie, prime annuelle, échéance et actions recommandées.', FileText],
  ['Tâches', 'Retard, priorité, client lié et statut de traitement.', Check],
  ['Morning Brief', 'Plan de journée piloté par ARK dès l’ouverture du cabinet.', Brain],
  ['Rapports', 'Portefeuille, activité, opportunités, échéances et rétention.', BarChart3],
  ['Admin', 'Accès sécurisé par rôle API/DB, sans contournement front.', ShieldCheck],
  ['Admin Costs', 'Pilotage des coûts ARK et suivi d’usage super_admin.', ShieldCheck],
  ['Clients à risque', 'Signaux de churn visibles avant qu’il ne soit trop tard.', AlertTriangle],
  ['Relances intelligentes', 'Actions prioritaires reliées au contexte métier.', Bell],
]

const integrations = [
  ['Google Agenda', 'Connexion prête pour synchroniser les rendez-vous et préparer les entretiens.', CalendarClock],
  ['WhatsApp Business', 'Threads clients centralisés, réponses plus rapides et suivi des non-traités.', MessageSquare],
  ['Gmail / Outlook', 'Architecture OAuth prête pour unifier les échanges email métier.', Mail],
  ['Webhooks Make / Zapier', 'Automations prêtes à connecter via webhooks entrants/sortants.', Zap],
]
...
const pricing = [
  ...
  },
  {
    ...
    note: 'Multi-utilisateurs, pilotage avancé et support prioritaire pour cabinets structurés.',
    href: '/register?plan=cabinet',
    cta: 'Activer mon essai Cabinet',
    featured: false,
    items: ['Tout Pro', '10 utilisateurs', 'Commissions et reporting avancé', 'Support prioritaire', 'Intégrations étendues'],
  },
  {
    name: 'Premium',
    price: 'Sur devis',
    period: '',
    label: 'Sur mesure',
    headline: 'Pour les déploiements complexes, réseaux et besoins spécifiques.',
    note: 'Accompagnement, intégrations avancées et trajectoire premium étudiés avec le cabinet.',
    externalHref: 'mailto:contact@courtia.fr?subject=COURTIA%20Premium',
    cta: 'Parler à COURTIA',
    featured: false,
    items: ['Tout Cabinet', 'Multi-cabinet', 'Intégrations sur mesure', 'Accompagnement dédié', 'Support prioritaire'],
  },
]
...
const faq = [
  ...
  ['COURTIA est-il pensé pour les courtiers français ?', 'Oui. Le vocabulaire, les écrans et les priorités sont conçus pour le courtage en assurance en France.'],
  ['Les intégrations Google Agenda et WhatsApp Business sont-elles prévues ?', 'Oui. Les connexions sont préparées avec un statut clair: activées uniquement quand votre configuration est en place.'],
...
export default function LandingPublic() {
  ...
  useEffect(() => {
    applySeo({
      title: 'COURTIA — Le cockpit IA connecté des courtiers en assurance',
      description: 'Centralisez vos clients, contrats, tâches, agenda, WhatsApp et priorités ARK. COURTIA transforme votre portefeuille en plan d’action quotidien.',
      canonicalPath: '/',
    })
  }, [])
  ...
  return (
    <div className="courtia-landing min-h-screen overflow-x-hidden text-white">
      ...
      <main className="stream-shell">
        ...
        <section id="story" className="landing-section landing-act pt-24 lg:pt-28">
          ...
          <div className="act-shell grid min-w-0 gap-10 lg:grid-cols-[minmax(0,0.88fr)_minmax(0,1.12fr)] lg:items-center">
            <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }} className="min-w-0">
              ...
              <p className="mt-3 text-sm font-bold text-white/54">Le cockpit IA des courtiers en assurance</p>
              <h1 className="cinema-title mt-6 max-w-5xl break-words text-white">
                Le cockpit IA connecté des courtiers en assurance
              </h1>
              <p className="mt-6 max-w-2xl text-lg leading-relaxed text-white/68">
                Centralisez vos clients, contrats, tâches, agenda, WhatsApp et priorités ARK. COURTIA transforme votre portefeuille en plan d’action quotidien.
              </p>
              <div className="mt-7 flex flex-col gap-3 sm:flex-row">
                <AuroraButton href="/demo" size="lg" icon={<ArrowRight size={17} />} className="w-full sm:w-auto">
                  Demander une démo
                </AuroraButton>
                <AuroraButton href="/demo" variant="secondary" size="lg" className="w-full sm:w-auto">
              ...
              <div className="conversion-strip mt-5 grid max-w-2xl grid-cols-1 gap-2 rounded-2xl border border-white/[0.08] p-2 backdrop-blur-2xl sm:grid-cols-3">
                {['Priorité ARK', 'Échéances à 30 jours', 'Relances du jour'].map((item) => (
              ...
              <div className="mt-6 grid max-w-2xl gap-2 sm:grid-cols-5">
                {['Pensé pour les courtiers français', 'IA native ARK', 'Google Agenda', 'WhatsApp Business', 'Portefeuille vivant'].map((item) => (
        ...
        <section id="solution" className="landing-section px-5 py-10">
          <div className="act-shell">
            <SectionIntro label="Solution COURTIA" title="COURTIA transforme votre portefeuille en plan d’action">
              ARK lit les signaux métier, priorise ce qui compte aujourd’hui et propose la prochaine action utile, sans blabla générique.
            </SectionIntro>
          </div>
        </section>
        ...
        <section id="ark" className="landing-section landing-act">
          <div className="act-shell">
            <div className="grid gap-8 lg:grid-cols-[0.92fr_1.08fr] lg:items-start">
              <div>
                <p className="scene-kicker">Problème courtier</p>
                <h2 className="scene-title mt-5 text-white">Le quotidien d’un courtier est saturé.</h2>
            ...
            <div id="cockpit" className="mt-12 grid gap-8 lg:grid-cols-[0.82fr_1.18fr] lg:items-center">
              <div>
                <p className="scene-kicker">Démo visuelle app</p>
            ...
            <div id="integrations" className="mt-14">
              <SectionIntro label="Intégrations" title="Agenda, WhatsApp et emails au même endroit" align="left">
                Google Agenda, WhatsApp Business et Gmail/Outlook sont préparés pour enrichir le cockpit. Aucune fausse connexion affichée: le statut dépend de votre configuration cabinet.
              </SectionIntro>
