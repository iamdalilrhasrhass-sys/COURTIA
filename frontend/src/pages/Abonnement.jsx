  ...
  return (
    <div className="border-b border-gray-200 py-5">
      <button onClick={() => setOpen(!open)} className="w-full flex justify-between items-center text-left">
        ...
        <span className={`transform transition-transform duration-200 ${open ? 'rotate-45' : 'rotate-0'}`}>
...
export default function Abonnement() {
  ...
  const plans = {
    monthly: [
      { id: 'starter', name: 'Starter', price: 39, badge: 'Fondateur', features: ['Jusqu\'à 100 clients', 'Scores & Segments', 'Module Tâches', 'Support Email'], popular: false },
      { id: 'pro', name: 'Pro', price: 69, badge: 'Populaire', features: ['Jusqu\'à 500 clients', 'Tout Starter', 'Assistant IA - ARK', 'Rapports avancés'], popular: true },
      { id: 'elite', name: 'Elite', price: 129, badge: 'Illimité', features: ['Clients illimités', 'Tout Pro', 'API & Intégrations', 'Support prioritaire'], popular: false },
    ],
    annually: [
      { id: 'starter', name: 'Starter', price: Math.round(39 * 12 * 0.8), per: '/an', badge: 'Fondateur', features: ['Jusqu\'à 100 clients', 'Scores & Segments', 'Module Tâches', 'Support Email'], popular: false },
      { id: 'pro', name: 'Pro', price: Math.round(69 * 12 * 0.8), per: '/an', badge: 'Populaire', features: ['Jusqu\'à 500 clients', 'Tout Starter', 'Assistant IA - ARK', 'Rapports avancés'], popular: true },
      { id: 'elite', name: 'Elite', price: Math.round(129 * 12 * 0.8), per: '/an', badge: 'Illimité', features: ['Clients illimités', 'Tout Pro', 'API & Intégrations', 'Support prioritaire'], popular: false },
    ]
  }
    ...
    <div className="min-h-screen bg-[#f9fafb] font-sans">
      <main className="py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-5xl mx-auto text-center">
          <h1 className="text-4xl md:text-5xl font-black tracking-tight bg-gradient-to-r from-[#2563eb] to-[#7c3aed] text-transparent bg-clip-text">
            Choisissez votre plan
          </h1>
          <p className="mt-4 text-lg text-gray-600 max-w-2xl mx-auto">
            Débloquez la puissance de l'IA pour votre cabinet de courtage. Simple, transparent et sans engagement.
          </p>
          ...
          <div className="mt-10 flex justify-center items-center gap-4">
            ...
            <button
              ...
              className={`relative inline-flex items-center h-7 rounded-full w-12 transition-colors duration-300 ease-in-out ${billingCycle === 'annually' ? 'bg-[#2563eb]' : 'bg-gray-300'}`}
            >
            ...
            <span className={`font-semibold ${billingCycle === 'annually' ? 'text-blue-600' : 'text-gray-500'}`}>
        ...
        <div className="mt-12 max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
          {currentPlans.map(plan => (
            <div key={plan.id} className={`bg-white rounded-2xl p-8 border ${plan.popular ? 'border-2 border-[#2563eb] shadow-2xl shadow-blue-500/20 scale-105' : 'border-gray-200 shadow-lg'}`}>
              <div className="flex justify-between items-start">
                <h3 className="text-xl font-bold text-gray-900">{plan.name}</h3>
                {plan.badge && (
                  <span className={`px-3 py-1 text-xs font-semibold rounded-full ${plan.popular ? 'bg-[#2563eb] text-white' : 'bg-blue-100 text-blue-700'}`}>{plan.badge}</span>
                )}
              </div>
              <p className="mt-6">
                <span className="text-5xl font-black text-gray-900">{plan.price}€</span>
                <span className="text-lg font-semibold text-gray-500">
                  {plan.per ? plan.per : '/mois'}
                </span>
              </p>
