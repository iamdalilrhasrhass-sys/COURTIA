...
const StatusBadge = ({ status }) => {
  ...
  else if (['inactif'].includes(s)) config = { label: 'Inactif', classes: 'bg-gray-100 text-gray-500' }
...
const ScoreBar = ({ score }) => {
    ...
    let color = '#10b981'
    if (s < 40) color = '#ef4444'
    else if (s < 70) color = '#f59e0b'
    return (
...
const SkeletonRow = () => (
  <tr className="animate-pulse"><td className="p-4"><div className="flex items-center gap-4"><div className="w-10 h-10 rounded-full bg-gray-200"></div><div><div className="w-24 h-4 bg-gray-200 rounded"></div><div className="w-32 h-3 mt-1.5 bg-gray-200 rounded"></div></div></div></td><td className="p-4"><div className="w-20 h-5 bg-gray-200 rounded-full"></div></td><td className="p-4"><div className="w-24 h-4 bg-gray-200 rounded"></div></td><td className="p-4"><div className="w-24 h-4 bg-gray-200 rounded"></div></td><td className="p-4"><div className="flex gap-2 justify-end"><div className="w-6 h-6 bg-gray-200 rounded"></div><div className="w-6 h-6 bg-gray-200 rounded"></div></div></td></tr>
)
...
export default function Clients() {
  ...
  const thClass = "p-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider select-none cursor-pointer hover:bg-gray-100 transition-colors"
    ...
    <div className="min-h-screen bg-[#fafafa] font-sans antialiased">
      <main className="p-8">
        <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div className="flex items-center gap-3"><h1 className="text-2xl font-bold text-gray-900">Clients</h1><span className="px-2.5 py-1 bg-gray-100 text-gray-600 text-sm font-semibold rounded-full">{clients.length}</span></div>
          <button onClick={() => navigate('/clients/new')} className="flex items-center justify-center gap-2 px-4 py-2 bg-gradient-to-r from-[#2563eb] to-[#7c3aed] text-white rounded-lg text-sm font-semibold cursor-pointer transition-all duration-200 ease-out hover:shadow-lg hover:shadow-blue-500/20 hover:-translate-y-px"><Plus size={16} />Nouveau client</button>
        </header>
        
        <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="relative flex-1 max-w-lg"><Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400" size={18} /><input type="text" placeholder="Rechercher un client..." value={search} onChange={e => { setSearch(e.target.value); setPage(1) }} className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-lg text-sm text-black focus:ring-2 focus:ring-[#2563eb] focus:border-[#2563eb] outline-none transition-all duration-200" /></div>
        ...
        <div className="bg-white border border-gray-100 rounded-xl overflow-x-auto" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.06), 0 4px 16px rgba(0,0,0,0.04)' }}>
          <table className="w-full text-sm min-w-[700px]">
            <thead className="bg-gray-50"><tr>{Object.keys(headerMapping).map(h => (<th key={h} onClick={() => toggleSort(headerMapping[h])} className={thClass}><div className="flex items-center gap-1.5">{h}<SortIcon active={sortField === headerMapping[h]} dir={sortDir} /></div></th>))}<th className="p-4 text-right text-xs font-semibold text-gray-400 uppercase tracking-wider">Actions</th></tr></thead>
            <tbody>
              {loading ? Array.from({ length: 10 }).map((_, i) => <SkeletonRow key={i} />) : paginatedClients.map((client) => (
                  <tr key={client.id} className="border-b border-gray-100 last:border-0 group hover:bg-blue-50/30 transition-colors duration-200">
                    <td className="p-4"><div className="flex items-center gap-4"><Avatar name={`${client.prenom || ''} ${client.nom || ''}`} /><div><p className="font-semibold text-gray-900 tracking-tight">{client.nom || '—'} {client.prenom || ''}</p><p className="text-gray-400">{client.email || '—'}</p></div></div></td>
                    ...
                    <td className="p-4"><div className="flex justify-end items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200"><button onClick={() => navigate(`/client/${client.id}`)} className="p-2 rounded-md hover:bg-gray-200 text-gray-500 hover:text-black" title="Voir"><Eye size={16} /></button><button onClick={() => navigate(`/clients/${client.id}/edit`)} className="p-2 rounded-md hover:bg-gray-200 text-gray-500 hover:text-black" title="Modifier"><Pencil size={16} /></button><button onClick={() => alert('Suppression non implémentée')} className="p-2 rounded-md hover:bg-red-100 text-gray-500 hover:text-red-600" title="Supprimer"><Trash2 size={16} /></button></div></td>
                  </tr>
                ))}
            </tbody>
          </table>
