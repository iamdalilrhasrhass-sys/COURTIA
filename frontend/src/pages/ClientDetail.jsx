...
Client actuel: ${client?.name || 'Inconnu'}
Contrats actifs: ${client?.contracts?.length || 0}
Statut: ${client?.status || 'Actif'}
${client?.notes ? `Notes: ${client.notes}` : ''}

Réponds en français, de façon concise et professionnelle. Montre ta valeur métier.`;
 ...
 { label: 'Rédige un email', prompt: `Rédige un email de relance professionnel pour ce client.` },
 { label: 'Détecte les risques', prompt: `Quels sont les risques de résiliation ou problèmes pour ce client ?` },
 { label: 'Opportunités cross-sell', prompt: `Quelles opportunités cross-sell pour ce client ? Sois concis.` }
 ];

 return (
 <div style={{ fontFamily: 'Arial, sans-serif', background: '#fff', minHeight: '100vh', padding: '40px' }}>
 <button onClick={() => navigate('/dashboard')} style={{ marginBottom: '20px', padding: '8px 16px', background: '#0a0a0a', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>← Retour</button>

 {client ? (
 <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px' }}>
 {/* CLIENT INFO */}
 <div>
 <h1 style={{ fontSize: '32px', fontWeight: 'bold', marginBottom: '20px' }}>{client.name}</h1>
 ...
 Parlez à ARK. Il connaît ce client.
 </div>
 )}
 {messages.map((msg, i) => (
 ...
 <button key={action.label} onClick={() => sendToARK(action.prompt)} disabled={loading || thinking} style={{ flex: '1 1 auto', minWidth: '90px', padding: '6px 10px', background: 'rgba(255,255,255,0.04)', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: '6px', color: 'rgba(255,255,255,0.5)', fontSize: '10px', fontWeight: '600', cursor: 'pointer', transition: 'all 0.2s' }} onMouseEnter={(e) => { e.target.style.background = 'rgba(255,255,255,0.08)'; e.target.style.color = 'rgba(255,255,255,0.8)'; }} onMouseLeave={(e) => { e.target.style.background = 'rgba(255,255,255,0.04)'; e.target.style.color = 'rgba(255,255,255,0.5)'; }}>
 {action.label}
 </button>
 ))}
 </div>
 ...
 <button onClick={() => sendToARK(input)} disabled={!input.trim() || loading || thinking} style={{ background: '#2563eb', borderRadius: '6px', padding: '8px 12px', fontSize: '11px', color: '#fff', border: 'none', cursor: 'pointer', fontWeight: '700' }}>Envoyer</button>
 </div>
 </div>
 </div>
 ) : (
