...
import { FileText, Shield, CheckSquare, Bot, ArrowLeft, Mail, Phone, MapPin, Building, Star, AlertTriangle, Calendar, User, Sparkles, Activity, Heart, Target, TrendingUp, ChevronDown } from 'lucide-react'
import api from '../api'
...
function BubbleTooltip({ type, client, scores, onMouseEnter, onMouseLeave }) {
  ...
  return (
    <div className="fixed inset-0 z-50 pointer-events-none flex items-center justify-center p-4">
      <motion.div initial={{ scale: 0.1, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.1, opacity: 0, filter: 'blur(8px)' }} transition={{ type: 'spring', stiffness: 180, damping: 14 }} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} style={{ pointerEvents: 'auto', background: 'rgba(255,255,255,0.25)', backdropFilter: 'blur(24px) saturate(200%)', border: '2px solid rgba(147,51,234,0.3)', boxShadow: '0 0 60px rgba(59,130,246,0.4), inset 0 0 30px rgba(147,51,234,0.1)', borderRadius: '40% 60% 50% 50% / 50% 50% 60% 40%', animation: 'bubbleFloat 6s ease-in-out infinite' }} className="relative w-full max-w-xl p-8 text-slate-900 max-h-[80vh] overflow-y-auto">
        <div className="flex justify-between items-start"><div className="flex items-center gap-3"><item.Icon size={24} /><h3 className="text-2xl font-black">{item.title}</h3></div>{item.coeff && <span className="text-xs font-semibold bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">Coeff. {item.coeff}</span>}</div>
        <div className="text-5xl font-black my-3">{item.score} <span className="text-3xl text-slate-400">/100</span></div>{item.content}
      </motion.div>
    </div>
  )
}
...
function ScoreSidebar({ scores, onBubbleEnter, onBubbleLeave }) {
  ...
  return (
    <aside className="w-[320px] flex-shrink-0 bg-slate-50/70 border-l border-slate-200/80 p-6 sticky top-0 h-screen overflow-y-auto">
      ...
      <ScoreLegend /><footer className="text-center mt-8"><p className="text-xs text-gray-300">Rhasrhass®</p></footer>
    </aside>
  )
}
...
export default function ClientDetail() {
  ...
  const TABS = [ { id: 'infos', label: 'Informations', icon: FileText, component: <InfosTab client={client}/> }, { id: 'contrats', label: 'Contrats', icon: Shield, component: <PlaceholderTab title="Contrats"/> }, { id: 'taches', label: 'Tâches', icon: CheckSquare, component: <PlaceholderTab title="Tâches"/> }, { id: 'ark', label: 'ARK Chat', icon: Bot, component: <PlaceholderTab title="ARK Chat"/> }]
