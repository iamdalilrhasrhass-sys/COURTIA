/**
 * arkWatch.portee-cabinet.test.js — la portée de GET /api/ark-watch/signals et DELETE /api/ark-watch/signals/:id est CELLE DU CABINET.
 *
 * POURQUOI CE TEST : ARK Watch surveille le PORTEFEUILLE DU CABINET : un collaborateur voyait 0
 * signal et 0 KPI, alors que les clients concernés sont ceux qu'il traite.
 *
...
jest.mock('../services/arkWatch', () => ({ runArkWatch: jest.fn(), getSignalStats: jest.fn(), generateMorningBrief: jest.fn(), getDetectorsList: () => [] }))
const pool = require('../db')
const router = require('./arkWatch')
...
const LIRE = () => gestionnaire('get', '/signals')
const SUPPRIMER = () => gestionnaire('delete', '/signals/:id')

describe('arkWatch — portée cabinet', () => {
  ...
  test('(b) compte SANS cabinet : clause historique sur le broker', async () => {
    ...
    expect(res.code).toBe(200)
    const sqls = sqlsDe(requetes)
    expect(sqls).toMatch(/s\.broker_id = \$1/)
    expect(sqls).not.toMatch(CLAUSE_CABINET)
  })

  test('(c) assistant : suppression d’un signal refusée (403), aucune écriture', async () => {
