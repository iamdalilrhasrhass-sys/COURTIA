/**
 * tags.portee-cabinet.test.js — la portée de GET /api/tags et POST /api/tags est CELLE DU CABINET.
 *
 * POURQUOI CE TEST : un tag est une classification du PORTEFEUILLE : un collaborateur ne voyait
 * ni les tags de ses collègues, ni ne pouvait classer un client du cabinet.
 *
...
const deux = require('./tags')

const routeur = deux.router
...
describe('tags — portée cabinet', () => {
  ...
  test('(a) broker du cabinet : la liste porte la clause CABINET', async () => {
    ...
    const liste = metier(requetes).find((r) => /FROM tags t/.test(r.sql))
    expect(liste.sql).toMatch(/\bcabinet_id = ANY\(\$1::uuid\[\]\)/)
    expect(liste.sql).toContain('FROM cabinet_members cm')
    expect(liste.sql).not.toMatch(CLAUSE_PROPRIETAIRE)
  ...
  test('(b) compte SANS cabinet : clause historique sur le courtier', async () => {
    ...
    expect(res.code).toBe(200)
    const liste = metier(requetes).find((r) => /FROM tags t/.test(r.sql))
    expect(liste.sql).toMatch(/t\.courtier_id = \$1/)
    expect(liste.sql).not.toMatch(/cabinet_id = ANY/)
    expect(liste.params).toEqual([7])
  })

  test('(c) assistant : création de tag refusée (403), aucune écriture', async () => {
    const { res, requetes } = await jouer(CREER(), [{ cabinet_id: CAB_A, role: 'assistant', retire: false }],
      { rows: [], rowCount: 0 }, { body: { name: 'À relancer', color: '#ff0000' } })
