...
router.get('/stats', verifyToken, async (req, res) => {
  try {
    ...
    const r4 = await pool.query("SELECT COUNT(*) as actifs, COALESCE(SUM(NULLIF(quote_data->>'prime_annuelle', '')::decimal),0) as prime_totale FROM quotes q JOIN clients c ON q.client_id=c.id WHERE q.status='actif' AND c.courtier_id=$1", [userId]);
    ...
    // réellement perçues par le cabinet.
    const r4c = await pool.query(`SELECT COALESCE(SUM(CASE WHEN COALESCE(received_amount_cents,0) <> 0 THEN received_amount_cents ELSE expected_amount_cents END),0)::numeric / 100 AS commissions, COUNT(*)::int AS nb FROM commissions WHERE user_id=$1 AND period_year=EXTRACT(YEAR FROM NOW()) AND period_month=EXTRACT(MONTH FROM NOW())`, [userId]);
    ...
    res.json({ totalClients:total, contratsActifs:parseInt(r4.rows[0].actifs), commissionsMois:parseFloat(r4c.rows[0].commissions||0), commissionsMoisSource:'commissions', commissionsMoisEnregistrees:parseInt(r4c.rows[0].nb||0), primeTotale:parseFloat(r4.rows[0].prime_totale||0), contratsUrgents:parseInt(r5.rows[0].count), tauxConversion:total>0?Math.round((actifs/total)*1000)/10:0, scoreRisqueMoyen:parseInt(r2.rows[0].score), clientsParStatut, clientsParSegment, revenus6Mois:r6.rows, alertes:r7.rows, clientsRecents:r8.rows, typesContrats:r9.rows });
  } catch(err) {
