...
router.get('/stats', verifyToken, async (req, res) => {
  try {
    ...
    const r4 = await pool.query("SELECT COUNT(*) as actifs, COALESCE(ROUND(SUM((quote_data->>'prime_annuelle')::decimal*0.15/12),2),0) as commissions, COALESCE(SUM((quote_data->>'prime_annuelle')::decimal),0) as prime_totale FROM quotes WHERE status='actif'");
    const r5 = await pool.query("SELECT COUNT(*) as count FROM quotes WHERE (quote_data->>'date_echeance')::date BETWEEN NOW() AND NOW()+INTERVAL '30 days' AND status='actif'");
    const r6 = await pool.query("SELECT TO_CHAR(DATE_TRUNC('month',created_at),'Mon') as mois, COALESCE(SUM((quote_data->>'prime_annuelle')::decimal),0) as revenue FROM quotes WHERE created_at>=NOW()-INTERVAL '6 months' AND status='actif' GROUP BY DATE_TRUNC('month',created_at) ORDER BY 1 ASC");
    ...
    const r9 = await pool.query("SELECT COALESCE(quote_data->>'type_contrat','Autre') as type,COUNT(*) as count,COALESCE(SUM((quote_data->>'prime_annuelle')::decimal),0) as total_primes FROM quotes WHERE status='actif' GROUP BY 1 ORDER BY 2 DESC");
