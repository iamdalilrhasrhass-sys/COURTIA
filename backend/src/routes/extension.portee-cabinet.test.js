/**
 * extension.portee-cabinet.test.js — la portée de POST /api/extension/fill est CELLE DU CABINET.
 *
 * POURQUOI CE TEST : l'extension Chrome est un poste de travail du courtier : elle doit pouvoir
 * pré-remplir le formulaire d'un client de son CABINET. La lecture retombait sur
 * `courtier_id = $1` et répondait « aucune donnée client en base pour ce cabinet »
 * sur les dossiers traités par un collègue.
 *
...
const CHAMPS = [{ name: 'nom', label: 'Nom' }]
const REMPLIR = () => gestionnaire('post', '/fill')

describe('extension — portée cabinet sur /fill', () => {
  beforeEach(() => pool.query.mockReset())

  test('(a) broker du cabinet : la lecture du client porte la clause CABINET', async () => {
    const { res, requetes } = await jouer(REMPLIR(), [{ cabinet_id: CAB_A, role: 'broker', retire: false }],
      { rows: [{ last_name: 'Durand' }], rowCount: 1 }, { body: { clientId: 7, fields: CHAMPS } })
    expect(res.code).toBe(200)
    const sqls = sqlsDe(requetes)
    expect(sqls).toMatch(CLAUSE_CABINET)
    expect(sqls).not.toMatch(CLAUSE_PROPRIETAIRE)
    // Le cabinet est passé en PARAMÈTRE (`$1::uuid[]`), pas recopié dans le SQL.
    const lecture = metier(requetes).find((r) => /FROM clients c/.test(r.sql))
    expect(lecture.params[0]).toEqual([CAB_A])
  })

  test('(b) compte SANS cabinet : clause historique sur le courtier', async () => {
    const { res, requetes } = await jouer(REMPLIR(), [], { rows: [], rowCount: 0 },
      { body: { clientId: 7, fields: CHAMPS } })
    expect(res.code).toBe(200)
    const sqls = sqlsDe(requetes)
  ...
  test('(c) assistant : la LECTURE reste autorisée (route de lecture, aucune écriture)', async () => {
    const { res, requetes } = await jouer(REMPLIR(), [{ cabinet_id: CAB_A, role: 'assistant', retire: false }],
      { rows: [], rowCount: 0 }, { body: { clientId: 7, fields: CHAMPS } })
    expect(res.code).toBe(200)
