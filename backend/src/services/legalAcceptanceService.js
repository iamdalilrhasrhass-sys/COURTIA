...
async function getLatestAcceptance(organizationId, userId, planCode) {
  ...
  const foundTrial = result.rows.find((r) => r.doc_type === 'trial_consent');
  ...
  if (!foundTrial || !foundRenewal || !foundCgv || !foundPrivacy || !foundDpa) {
    return null;
  }
  ...
  return {
    ...
    docs: [foundCgv, foundPrivacy, foundDpa, foundTrial, foundRenewal],
  };
}
